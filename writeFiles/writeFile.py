...
def getTrajectory(API_KEY, mode, waypoints, thershold, number_of_trajectory, filename):
    ...
    with open(filename + '.csv', 'w') as csvfile:
        ...
        for i in range(0, number_of_trajectory):
            trajectory = generateTrajectory(API_KEY, i, mode, thershold, waypoints)
...
def generateStartEnd(mode, thershold):
    ...
    while (start[0] == end[0] or start[1] == end[0]):
        end = calculation.generateRandom(_lower_boundary_coordinate, _upper_boundary_coordinate)
...
def generateTrajectory(API_KEY, id, mode, thershold, waypoints):
